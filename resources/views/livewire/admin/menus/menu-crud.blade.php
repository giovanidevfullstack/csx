        ...
        <form wire:submit.prevent="update" class="h-full">
            <div class="w-full h-full flex flex-col p-5 space-y-2 text-indigo-400">
                ...
                <div class="inline-block space-x-2">
                    <label class="text-gray-500 dark:text-gray-400 ">Administração?</label>
                    <input type="checkbox" wire:model.defer="menu.is_admin" {{ $menu['is_admin'] ? 'checked' : '' }}
                            class="rounded border-gray-300 text-indigo-400 focus:ring-indigo-400 dark:bg-gray-900">
                    @error('menu.is_admin') <span class="text-red-400 text-sm">{{ $message }}</span> @enderror

                    <label class="text-gray-500 dark:text-gray-400 ">Ativado</label>
                ...
                <label class="text-gray-50 dark:text-gray-400 0">Rota</label>
